...
$record_count = $link->query("SELECT * FROM posts WHERE topic_id='".$topic_id."'");
$per_page = 5;
...
$prev = $page - 1;
$next = $page + 1;
$query = "SELECT * FROM posts  WHERE topic_id='".$topic_id."' order by post_id desc limit $start, $per_page";
...
<body>
    ...
    <div class="row">
        <div class="leftcolumn">
            <?php
                $record = $link->query("SELECT * FROM topics WHERE topic_id='".$topic_id."'");
                $row = $record->fetch_object();
                echo "<div class='post-preview'>";
                echo "<div class='post-container'>";
                echo "<h2>Results of ".$row->toname.":</h2>";
            ...
            <?php
             if($prev > 0) {
                 echo "<a class='pre-button' href='topic.php?topic_id=$topic_id&p=$prev'>« Previous</a>";
             }
             if($page < $pages) {
                echo "<a class='next-button' href='topic.php?topic_id=$topic_id&p=$next'>Next »</a>";
             }
            ?>
        </div>
